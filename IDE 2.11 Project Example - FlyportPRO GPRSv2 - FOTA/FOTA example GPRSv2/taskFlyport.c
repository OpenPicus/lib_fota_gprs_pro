...
ActionStateMachine actionState = GSM_NET_UP;
...
void FlyportTask()
{
    ...
    _dbgline("\r\n\tFlyportPRO GPRSv2 FOTA Example FW1\r\n");
#else
    _dbgline("\r\n\tFlyportPRO GPRSv2 FOTA Example FW2\r\n");
#endif
	xTaskResumeAll();
...
#ifdef _FW_1_FOTA_GPRS_
					_dbgwrite("/FOTA_GPRSv2_2/");
					_dbgline(fileName);
					FTPReceive(&ftpSocket, fileName, "/FOTA_GPRSv2_2", fileName);
#else
					_dbgwrite("/FOTA_GPRSv2_1/");
					_dbgline(fileName);
					FTPReceive(&ftpSocket, fileName, "/FOTA_GPRSv2_1", fileName);
#endif
