	...
	switch(smInternal)
	{
		...
		case 1:	
			...
			if(ackSMSrequested == FALSE)
			{
				...
				sprintf(msg2send, "AT+CSMP=17,167,0,0\r"); // Sets valid period, PDU mode and Char Set as default
			}
			else
			{
				// set +CSMP param (AT+CSMP=49,167,0,0)
				sprintf(msg2send, "AT+CSMP=49,167,0,0\r"); // Add the ACK Request flag to default SMS send params
			}
		...
		case 9:
			...
			sprintf(msg2send, "\r\n+CMG");
			chars2read = 2;
			countData = 0; // GSM buffer should be: <CR><LF>+CMGS: <mr><CR><LF>
            maxtimeout = 60*3 + 2; // up to 3 minutes for SMS send operation for ublox modules
            tick = TickGetDiv64K(); // 1 tick every seconds
...
int cSMSRead()
{
	...
	switch (smInternal)
	{
		...
		case 3:
			...
			sprintf(msg2send, "\r\n+CPMS");
			chars2read = 2;
			countData = 0; // GSM buffer should be: <CR><LF>+CPMS...<CR><LF>
            tick = TickGetDiv64K(); // 1 tick every seconds
			maxtimeout = 60*3 + 2; // up to 3 minutes of timeout for such response!
			resCheck = CheckCmd(countData, chars2read, tick, cmdReply, msg2send, maxtimeout);
            ...
            else
            {
                ...
                // TODO: use +CMPS response...
                // +CMPS response parameters can be used to know how many SMS are present in memory,
                // and the max number of SMS that can be stored...
            }
		...
		case 10:
			...
			sprintf(msg2send, "\r\n+CMGR:");
			chars2read = 2;
			countData = 0; // GSM buffer should be: <CR><LF>+CMGR:...params...<CR><LF>
            tick = TickGetDiv64K(); // 1 tick every seconds
			maxtimeout = 60*3 + 2; // up to 3 minutes of timeout for such response!
			resCheck = CheckCmd(countData, chars2read, tick, cmdReply, msg2send, maxtimeout);
...
int cSMSDelete()
{
	...
	switch (smInternal)
	{
		...
		case 5:
			...
			if(mainSMS.Index <= 0)
			{
				if(mainSMS.Index < SMS_DEL_READ_ALL_MEMORY)
                {
                ...
                {
                    // Special command for SMS Delete
                    // Write AT+CMGD=0,<flag>\r
                    sprintf(msg2send, "AT+CMGD=0,%d\r", -mainSMS.Index);
                }
			}
            else
            {
